...
export async function getBookkeepingData(year, month, rangeStart = null, rangeEnd = null) {
  let dateFilter = "";
  let startDate = null;
  let endDate = null;
  ...
  const [settings, accounts, categories, transactions, payroll, contractors, contractorPayments, invoices, installments, ownerRow] = await Promise.all([
    sql`SELECT company_name, tax_state, tax_local_rate, tax_filing FROM settings WHERE id = 1`,
    ...
    sql`SELECT id, number, client_name, total, amount_paid, deposit, status, due_date, recurring, recurring_next FROM invoices ORDER BY created_at DESC`,
    sql`SELECT id, invoice_id, amount, due_date, status FROM invoice_installments ORDER BY invoice_id, sort_order`,
    sql`SELECT COUNT(*)::int AS owners FROM users WHERE role = 'owner'`,
  ]);

  // Year-to-date profit on the same basis as the P&L: paid invoices + unlinked ledger income − expenses
  const taxYear = year || new Date().getFullYear();
  const [ytd] = await sql`
    SELECT
      COALESCE((SELECT SUM(total) FROM invoices WHERE status = 'paid' AND COALESCE(paid_at, created_at::date) BETWEEN ${`${taxYear}-01-01`} AND ${`${taxYear}-12-31`}), 0) AS invoice_revenue,
      COALESCE((SELECT SUM(amount) FROM bk_transactions WHERE type = 'income' AND invoice_id IS NULL AND date BETWEEN ${`${taxYear}-01-01`} AND ${`${taxYear}-12-31`}), 0) AS other_income,
      COALESCE((SELECT SUM(ABS(amount)) FROM bk_transactions WHERE type = 'expense' AND date BETWEEN ${`${taxYear}-01-01`} AND ${`${taxYear}-12-31`}), 0) AS expenses
  `;
  ...
  return {
    companyName: settings[0]?.company_name || "My Company",
    tax: {
      year: taxYear,
      state: settings[0]?.tax_state || 'MD',
      localRate: settings[0]?.tax_local_rate == null ? 3.2 : parseFloat(settings[0].tax_local_rate),
      filing: settings[0]?.tax_filing || 'single',
      owners: ownerRow[0]?.owners || 1,
      invoiceRevenue: parseFloat(ytd.invoice_revenue), otherIncome: parseFloat(ytd.other_income), expenses: parseFloat(ytd.expenses),
      netIncomeYtd: parseFloat(ytd.invoice_revenue) + parseFloat(ytd.other_income) - parseFloat(ytd.expenses),
    },
    accounts: accounts.map(a => ({ id: a.id, name: a.name, accountType: a.account_type })),
    invoices: invoices.map(i => ({ id: i.id, number: i.number, clientName: i.client_name, total: parseFloat(i.total), amountPaid: parseFloat(i.amount_paid), deposit: parseFloat(i.deposit || 0), status: i.status, dueDate: toISODate(i.due_date), recurring: i.recurring || '', recurringNext: toISODate(i.recurring_next), installments: installments.filter(x => x.invoice_id === i.id).map(x => ({ id: x.id, amount: parseFloat(x.amount), dueDate: toISODate(x.due_date), status: x.status })) })),
...
export async function getBalanceSheetData(asOfDate) {
  const [accountBalances, unassignedRow, otherIncomeRow, expensesRow, paidInvoicesRow, investmentsRow] = await Promise.all([
    sql`
      SELECT a.id, a.name, a.account_type,
             COALESCE(SUM(t.amount), 0) AS balance
      FROM bk_accounts a
      LEFT JOIN bk_transactions t ON t.account_id = a.id AND t.date <= ${asOfDate}
    ...
    sql`
      ...
      WHERE account_id IS NULL AND date <= ${asOfDate}
    `,
    sql`
      SELECT COALESCE(SUM(amount), 0) AS total
      FROM bk_transactions
      WHERE type = 'income' AND invoice_id IS NULL AND date <= ${asOfDate}
    `,
    sql`
      SELECT COALESCE(SUM(ABS(amount)), 0) AS total
      FROM bk_transactions
      WHERE type = 'expense' AND date <= ${asOfDate}
    `,
    sql`
      SELECT COALESCE(SUM(total), 0) AS revenue
      FROM invoices
      WHERE status = 'paid'
        AND COALESCE(paid_at, created_at::date) <= ${asOfDate}
    `,
    sql`
      SELECT COALESCE(SUM(CASE WHEN amount > 0 THEN amount ELSE 0 END), 0) AS contributions,
             COALESCE(SUM(CASE WHEN amount < 0 THEN -amount ELSE 0 END), 0) AS draws
      FROM bk_transactions
      WHERE type = 'equity' AND date <= ${asOfDate}
    `,
  ]);

  const invoiceRevenue = parseFloat(paidInvoicesRow[0]?.revenue || 0);
  const otherIncome = parseFloat(otherIncomeRow[0]?.total || 0); // Stripe plans, misc deposits — cash received that isn't an app invoice
  const totalExpenses = parseFloat(expensesRow[0]?.total || 0);
  const ownerContributions = parseFloat(investmentsRow[0]?.contributions || 0);
  const ownerDraws = parseFloat(investmentsRow[0]?.draws || 0);
  const ownerInvestments = ownerContributions - ownerDraws;

  return {
    accountBalances: accountBalances.map(a => ({
      id: a.id, name: a.name, accountType: a.account_type, balance: parseFloat(a.balance),
    })),
    unassignedBalance: parseFloat(unassignedRow[0]?.total || 0),
    basis: "cash",
    invoiceRevenue,
    otherIncome,
    totalExpenses,
    ...
    ownerDraws,
    ownerInvestments,
    retainedEarnings: invoiceRevenue + otherIncome - totalExpenses,
  };
}

// ── P&L DATA ──
export async function getPnLData(startDate, endDate) {
  const [categoryRows, invoiceRow] = await Promise.all([
    sql`
    ...
    sql`
      ...
      FROM invoices
      WHERE status = 'paid'
        AND COALESCE(paid_at, created_at::date) >= ${startDate}
        AND COALESCE(paid_at, created_at::date) <= ${endDate}
    `,
  ]);
  ...
  return {
    ...
    invoiceRevenue: parseFloat(invoiceRow[0]?.revenue || 0),
    invoiceCount: parseInt(invoiceRow[0]?.count || 0),
  };
}
...
export async function upsertTransaction(t) {
  await sql`
    INSERT INTO bk_transactions (id, date, description, name, amount, category_id, account_id, type, vendor, reference, notes, reconciled, reviewed, source, import_batch_id, invoice_id)
    VALUES (${t.id}, ${toISODate(t.date)}, ${t.description || ""}, ${t.name || ""}, ${t.amount}, ${t.categoryId || null}, ${t.accountId || null}, ${t.type || "expense"}, ${t.vendor || ""}, ${t.reference || ""}, ${t.notes || ""}, ${t.reconciled || false}, ${t.reviewed || false}, ${t.source || "manual"}, ${t.importBatchId || null}, ${t.invoiceId || null})
