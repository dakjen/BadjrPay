...
export async function getBookkeepingData(year, month) {
  ...
  const [settings, accounts, categories, transactions, payroll, contractors, contractorPayments, invoices, installments] = await Promise.all([
    sql`SELECT company_name, tax_rate FROM settings WHERE id = 1`,
  ...
  return {
    ...
    taxRate: settings[0]?.tax_rate == null ? 25 : parseFloat(settings[0].tax_rate),
