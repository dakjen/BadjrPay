...
export async function getBookkeepingData(year, month) {
  ...
  const [settings, accounts, categories, transactions, payroll, contractors, contractorPayments, invoices] = await Promise.all([
    ...
    sql`SELECT id, number, client_name, total, amount_paid, status, due_date FROM invoices ORDER BY created_at DESC`,
  ]);
  ...
  return {
    ...
    invoices: invoices.map(i => ({ id: i.id, number: i.number, clientName: i.client_name, total: parseFloat(i.total), amountPaid: parseFloat(i.amount_paid), status: i.status, dueDate: toISODate(i.due_date) })),
