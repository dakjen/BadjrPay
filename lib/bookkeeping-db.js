...
export async function getBalanceSheetData(asOfDate) {
  const [accountBalances, unassignedRow, arRows, expensesRow, paidInvoicesRow, investmentsRow] = await Promise.all([
    sql`
    ...
    `,
    sql`
      SELECT number, client_name, due_date, total, amount_paid, (total - amount_paid) AS outstanding
      FROM invoices
      WHERE status NOT IN ('paid', 'draft')
        AND (total - amount_paid) > 0
      ORDER BY client_name, due_date
    `,
    sql`
  ...
  return {
    ...
    ownerDraws,
    ownerInvestments,
    retainedEarnings: invoiceRevenue - totalExpenses,
  };
}
