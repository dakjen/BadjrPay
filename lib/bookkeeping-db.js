...
export async function getBookkeepingData(year, month) {
  ...
  const [settings, accounts, categories, transactions, payroll, contractors, contractorPayments, invoices, installments, ownerRow] = await Promise.all([
    ...
    sql`SELECT id, number, client_name, total, amount_paid, deposit, status, due_date FROM invoices ORDER BY created_at DESC`,
  ...
  return {
    ...
    invoices: invoices.map(i => ({ id: i.id, number: i.number, clientName: i.client_name, total: parseFloat(i.total), amountPaid: parseFloat(i.amount_paid), deposit: parseFloat(i.deposit || 0), status: i.status, dueDate: toISODate(i.due_date), installments: installments.filter(x => x.invoice_id === i.id).map(x => ({ id: x.id, amount: parseFloat(x.amount), dueDate: toISODate(x.due_date), status: x.status })) })),
