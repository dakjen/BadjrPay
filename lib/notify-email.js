...
const button = (href, label) => `<table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="margin:24px 0 8px;"><tr><td align="center"><a href="${escHtml(href)}" style="display:inline-block;background-color:#2D5A3D;color:#ffffff;text-decoration:none;font-size:15px;font-weight:600;padding:14px 32px;border-radius:8px;">${escHtml(label)}</a></td></tr></table>`;
...
<table role="presentation" width="600" cellspacing="0" cellpadding="0" border="0" style="max-width:600px;width:100%;background-color:#ffffff;border-radius:12px;overflow:hidden;border:1px solid #f0dda0;">
  <tr><td style="background-color:#ffbd5a;padding:24px 32px;"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0"><tr>
...
function ownerShell(title, body) {
  ...
  <div style="max-width:560px;margin:0 auto;background:#fff;border:1px solid #E2DDD3;border-radius:12px;overflow:hidden;">
    <div style="background:#2D5A3D;color:#fff;padding:14px 20px;font-weight:700;">Badjr-Pay</div>
...
export function buildOwnerAlert({ title, lines, link, linkLabel }) {
  return ownerShell(title, `<ul style="margin:0 0 16px;padding-left:18px;">${lines.map(l => `<li>${l}</li>`).join("")}</ul>${link ? `<a href="${escHtml(link)}" style="color:#2D5A3D;font-weight:600;">${escHtml(linkLabel || "Open Badjr-Pay")} →</a>` : ""}`);
}
export function buildOwnerDigest({ weekLabel, overdue, dueThisWeek, expected30, mrr, renewals, link }) {
  ...
  return ownerShell(`Week of ${weekLabel}`, `
    ...
    <h3 style="margin:14px 0 6px;font-size:14px;color:#B5342B;">Overdue (${overdue.length})</h3>${list(overdue, "Nothing overdue — nice.")}
    <h3 style="margin:14px 0 6px;font-size:14px;color:#C4841D;">Due this week (${dueThisWeek.length})</h3>${list(dueThisWeek, "Nothing due this week.")}
    ${renewals.length ? `<h3 style="margin:14px 0 6px;font-size:14px;color:#2B5EA7;">Renewals coming up</h3>${list(renewals, "")}` : ""}
    <p style="margin:16px 0 0;"><a href="${escHtml(link)}" style="color:#2D5A3D;font-weight:600;">Open Badjr-Pay →</a></p>`);
}
