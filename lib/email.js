...
// Sender: BREVO_SENDER="Name <email>" wins, else SENDER_NAME + SENDER_EMAIL
export function senderIdentity() {
  const combo = process.env.BREVO_SENDER || process.env.MAIL_FROM || "";
  const m = /^\s*(?:"?([^"<]*)"?\s*)?<([^>]+)>\s*$/.exec(combo);
  if (m) return { email: m[2].trim(), name: (m[1] || "").trim() || m[2].trim() };
  if (combo.includes("@")) return { email: combo.trim(), name: process.env.SENDER_NAME || combo.trim() };
  const email = process.env.SENDER_EMAIL || "";
  return { email, name: process.env.SENDER_NAME || email };
}

export async function sendMail({ to, toName, subject, html, attachments, templateId, templateData }) {
  const { email: senderEmail, name: senderName } = senderIdentity();
  if (!senderEmail) throw new Error("No sender configured — set BREVO_SENDER=\"Name <email>\"");
