...
// Neon hands DATE columns back as JS Date objects; normalize anything to YYYY-MM-DD
const toISODate = (d) => {
  if (!d) return null;
...
export async function getCompanySettings() {
  const rows = await sql`SELECT company_name, company_address, company_phone, auto_reminders FROM settings WHERE id = 1`;
  const s = rows[0] || {};
  return { companyName: s.company_name || "", companyAddress: s.company_address || "", companyPhone: s.company_phone || "", autoReminders: s.auto_reminders !== false };
}
...
export async function getInvoiceByPayToken(token) {
  ...
  return {
    ...
    invoice: {
      ...
      items: items.map(i => ({ description: i.description, qty: parseFloat(i.qty), rate: parseFloat(i.rate), amount: parseFloat(i.amount) || parseFloat(i.qty) * parseFloat(i.rate) })),
...
// ── INTEGRATION SETTINGS (e.g. the Stripe webhook signing secret, when set up from inside the app) ──
export async function initIntegrationDb() {
  await sql`CREATE TABLE IF NOT EXISTS integration_settings (key TEXT PRIMARY KEY, value TEXT NOT NULL, updated_at TIMESTAMPTZ DEFAULT NOW())`;
}
...
export async function setIntegrationSetting(key, value) {
  await sql`INSERT INTO integration_settings (key, value) VALUES (${key}, ${value}) ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value, updated_at = NOW()`;
}

// ── FULL INVOICE (server-side, same shape the app uses) ──
export async function getInvoiceById(id) {
  const rows = await sql`SELECT * FROM invoices WHERE id = ${id}`;
  const inv = rows[0];
  if (!inv) return null;
  const [items, installments] = await Promise.all([
    sql`SELECT description, qty, rate, amount FROM invoice_items WHERE invoice_id = ${id} ORDER BY sort_order`,
    sql`SELECT id, amount, due_date, status, paid_at FROM invoice_installments WHERE invoice_id = ${id} ORDER BY sort_order`,
  ]);
  return {
    id: inv.id, number: inv.number, clientId: inv.client_id || "", clientName: inv.client_name, clientEmail: inv.client_email, clientPhone: inv.client_phone, clientAddress: inv.client_address,
    projectId: inv.project_id, dueDate: toISODate(inv.due_date) || "", createdAt: toISODate(inv.created_at) || "", sentAt: toISODate(inv.sent_at) || "", paidAt: toISODate(inv.paid_at) || "",
    notes: inv.notes, status: inv.status, amountPaid: parseFloat(inv.amount_paid), total: parseFloat(inv.total), deposit: parseFloat(inv.deposit || 0),
    recurring: inv.recurring || "", recurringNext: toISODate(inv.recurring_next) || "",
    items: items.map(i => ({ description: i.description, qty: parseFloat(i.qty), rate: parseFloat(i.rate), amount: parseFloat(i.amount) || parseFloat(i.qty) * parseFloat(i.rate) })),
    installments: installments.map(i => ({ id: i.id, amount: parseFloat(i.amount), dueDate: toISODate(i.due_date), status: i.status, paidAt: toISODate(i.paid_at) })),
  };
}
...
export async function listOpenInvoices() {
  const rows = await sql`SELECT id FROM invoices WHERE status <> 'draft' AND total - amount_paid > 0.005 ORDER BY due_date NULLS LAST`;
  return Promise.all(rows.map(r => getInvoiceById(r.id)));
}

export async function listRecurringInvoices() {
  const rows = await sql`SELECT id FROM invoices WHERE recurring IS NOT NULL AND recurring <> '' AND recurring_next IS NOT NULL`;
  return Promise.all(rows.map(r => getInvoiceById(r.id)));
}

export async function markInvoiceOverdue(id) {
  await sql`UPDATE invoices SET status = 'overdue' WHERE id = ${id} AND status IN ('sent', 'viewed')`;
}

// Create the next invoice of a recurring series (server-side). The recurrence moves to the new invoice.
export async function createRenewalInvoice(src, { dueDate, recurringNext }) {
  const nums = await sql`SELECT number FROM invoices`;
  const maxN = nums.reduce((m, r) => Math.max(m, parseInt(String(r.number).replace(/\D/g, "")) || 0), 0);
  const id = genId();
  const number = `INV-${String(Math.max(maxN + 1, nums.length + 1)).padStart(4, "0")}`;
  await sql`
    INSERT INTO invoices (id, number, client_id, client_name, client_email, client_phone, client_address, project_id, due_date, notes, status, amount_paid, total, deposit, sent_at, recurring, recurring_next)
    VALUES (${id}, ${number}, ${src.clientId || null}, ${src.clientName || ""}, ${src.clientEmail || ""}, ${src.clientPhone || ""}, ${src.clientAddress || ""}, ${src.projectId || ""}, ${dueDate}, ${src.notes || ""}, 'sent', 0, ${src.total || 0}, 0, CURRENT_DATE, ${src.recurring}, ${recurringNext})
  `;
  for (let i = 0; i < (src.items || []).length; i++) {
    const it = src.items[i];
    await sql`INSERT INTO invoice_items (invoice_id, description, qty, rate, amount, sort_order) VALUES (${id}, ${it.description || ""}, ${it.qty || 1}, ${it.rate || 0}, ${it.amount || 0}, ${i})`;
  }
  await sql`UPDATE invoices SET recurring = NULL, recurring_next = NULL WHERE id = ${src.id}`;
  return getInvoiceById(id);
}

// ── EMAIL LOG (send each automated email once) ──
...
export async function alreadySent(scope, key) {
  const rows = await sql`SELECT 1 FROM email_log WHERE scope = ${scope} AND key = ${key}`;
  return rows.length > 0;
}
export async function logSent(scope, key) {
  await sql`INSERT INTO email_log (scope, key) VALUES (${scope}, ${key}) ON CONFLICT DO NOTHING`;
}

// ── INVOICE ATTACHMENTS (contracts, proposals — files live in Vercel Blob) ──
export async function initAttachmentsDb() {
  await sql`
...
const mapAttachment = (r) => ({ id: r.id, invoiceId: r.invoice_id, filename: r.filename, url: r.url, size: r.size, contentType: r.content_type, createdAt: r.created_at ? new Date(r.created_at).toISOString() : null });
export async function addAttachment(a) {
  const rows = await sql`INSERT INTO invoice_attachments (id, invoice_id, filename, url, size, content_type, uploaded_by) VALUES (${genId()}, ${a.invoiceId}, ${a.filename}, ${a.url}, ${a.size || 0}, ${a.contentType || ""}, ${a.uploadedBy || ""}) RETURNING *`;
  return mapAttachment(rows[0]);
}
export async function getAttachment(id) { const r = await sql`SELECT * FROM invoice_attachments WHERE id = ${id}`; return r[0] ? mapAttachment(r[0]) : null; }
export async function deleteAttachment(id) { await sql`DELETE FROM invoice_attachments WHERE id = ${id}`; }
export async function listAttachmentsByInvoice() {
  const rows = await sql`SELECT * FROM invoice_attachments ORDER BY created_at`;
  const by = {}; for (const r of rows) (by[r.invoice_id] ||= []).push(mapAttachment(r)); return by;
}
export async function listAttachments(invoiceId) { return (await sql`SELECT * FROM invoice_attachments WHERE invoice_id = ${invoiceId} ORDER BY created_at`).map(mapAttachment); }
