...
// Re-apply confirmed online payments to invoices so a stale client-side save can never undo them.
  ...
  await sql`
    UPDATE invoices i SET
      ...
      status = CASE
        ...
        ELSE i.status END,
      paid_at = CASE WHEN GREATEST(i.amount_paid, agg.paid) >= i.total THEN COALESCE(i.paid_at, agg.last_paid) ELSE i.paid_at END
    FROM (
      SELECT p.invoice_id, SUM(p.amount) AS paid, MAX(p.paid_at) AS last_paid
      FROM invoice_online_payments p WHERE p.status = 'paid' AND (${invoiceId}::text IS NULL OR p.invoice_id = ${invoiceId})
