...
export function drawInvoicePDF(invoice, settings, client) {
  ...
  // Header bar — brand gold with the green wordmark (same look as the emails)
  doc.setFillColor(...GOLD);
  doc.rect(0, 0, W, 44, "F");
  let textX = margin;
  try { const lw = 40, lh = lw / LOGO_ASPECT; doc.addImage(LOGO_PNG_DATA_URL, "PNG", margin, 22 - lh / 2, lw, lh); textX = margin + lw + 8; } catch (_) {}
  doc.setTextColor(...A);
