...
import { LOGO_PNG_DATA_URL, LOGO_ASPECT } from "./logo-data";
...
export function drawInvoicePDF(invoice, settings, client) {
  ...
  let y = 20;
  const A = [71, 108, 46], D = [26, 26, 26], G = [107, 101, 96], L = [237, 233, 225], GOLD = [255, 189, 89], NAVY = [11, 45, 101];

  // Header — navy brand stripe, green wordmark, navy company name (green + navy scheme)
  doc.setFillColor(...NAVY);
  doc.rect(0, 0, W, 3, "F");
  doc.setDrawColor(...L); doc.line(0, 44, W, 44);
  let textX = margin;
  try { const lw = 40, lh = lw / LOGO_ASPECT; doc.addImage(LOGO_PNG_DATA_URL, "PNG", margin, 23 - lh / 2, lw, lh); textX = margin + lw + 8; } catch (_) {}
  doc.setTextColor(...NAVY);
  doc.setFont("helvetica", "bold"); doc.setFontSize(16);
  doc.text(settings.companyName || "BaDjR Tech", textX, 19);
  doc.setTextColor(...D);
  doc.setFont("helvetica", "normal"); doc.setFontSize(8.5);
  if (settings.companyAddress) doc.text(settings.companyAddress, textX, 26);
  if (settings.companyPhone) doc.text(settings.companyPhone, textX, 31.5);
  ...
  doc.text("Thank you for your business.", W / 2, 266, { align: "center" });
