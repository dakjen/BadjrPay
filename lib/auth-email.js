...
export function buildLoginCodeEmail({ code, name }) {
  return shell({ title: "Your sign-in code", badge: "Sign in", body: `
    ...
    <div style="text-align:center;margin:8px 0 20px;"><span style="display:inline-block;font-size:34px;font-weight:700;letter-spacing:0.28em;color:#476C2E;background:#EAF1E3;padding:14px 22px 14px 30px;border-radius:10px;font-family:'Courier New',monospace;">${escHtml(code)}</span></div>
...
export function buildResetEmail({ link, name }) {
  return shell({ title: "Reset your password", badge: "Password reset", body: `
    ...
    <div style="text-align:center;margin:8px 0 20px;"><a href="${escHtml(link)}" style="display:inline-block;background-color:#476C2E;color:#ffffff;text-decoration:none;font-size:15px;font-weight:600;padding:13px 28px;border-radius:8px;">Choose a new password</a></div>
    <p style="margin:0;font-size:12px;color:#9C9590;word-break:break-all;">Or paste this into your browser:<br/><a href="${escHtml(link)}" style="color:#476C2E;">${escHtml(link)}</a></p>` });
}
