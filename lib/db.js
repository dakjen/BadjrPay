...
export async function getAppData() {
  ...
  const [settingsRows, clients, categories, services, invoiceRows, itemRows, installmentRows] = await Promise.all([
  ...
  for (const inv of invs) {
    ...
    for (let i = 0; i < installments.length; i++) {
      ...
      await sql`
        ...
        VALUES (${inst.id}, ${inv.id}, ${inst.amount}, ${inst.dueDate}, ${inst.status || 'pending'}, ${inst.paidAt || null}, ${i})
        ON CONFLICT (id) DO UPDATE SET
