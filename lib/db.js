...
import { initBillingDb, reconcileOnlinePayments, listOnlinePaymentsByInvoice, listAttachmentsByInvoice } from "./billing-db";
import { initAuthDb } from "./auth-codes";
import { initProductsDb, listProductLines } from "./products-db";
...
export async function initDb() {
  ...
  await sql`ALTER TABLE settings ADD COLUMN IF NOT EXISTS tax_rate NUMERIC(5,2) DEFAULT 25`;
  await sql`ALTER TABLE settings ADD COLUMN IF NOT EXISTS tax_state TEXT DEFAULT 'MD'`;
  await sql`ALTER TABLE settings ADD COLUMN IF NOT EXISTS tax_local_rate NUMERIC(5,3) DEFAULT 3.2`;
  await sql`ALTER TABLE settings ADD COLUMN IF NOT EXISTS tax_filing TEXT DEFAULT 'single'`;
  await sql`ALTER TABLE settings ADD COLUMN IF NOT EXISTS auto_reminders BOOLEAN DEFAULT TRUE`;
  await sql`
  ...
  await sql`ALTER TABLE invoices ADD COLUMN IF NOT EXISTS recurring TEXT`;
  await sql`ALTER TABLE invoices ADD COLUMN IF NOT EXISTS recurring_next DATE`;
  // Status columns are enums so the DB itself only allows real values (and Neon's table editor shows a dropdown)
  await sql`
    DO $$ BEGIN
      IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'invoice_status') THEN
        CREATE TYPE invoice_status AS ENUM ('draft', 'sent', 'viewed', 'partial', 'paid', 'overdue');
      END IF;
      IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'invoices' AND column_name = 'status' AND data_type = 'text') THEN
  ...
  await sql`
    DO $$ BEGIN
      ...
      END IF;
      IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'invoice_installments' AND column_name = 'status' AND data_type = 'text') THEN
        ALTER TABLE invoice_installments ALTER COLUMN status DROP DEFAULT;
        ALTER TABLE invoice_installments ALTER COLUMN status TYPE installment_status USING status::installment_status;
        ALTER TABLE invoice_installments ALTER COLUMN status SET DEFAULT 'pending';
      END IF;
    END $$
  `;
  await sql`
  ...
  const seedCats = [
    ...
    { id: 'bkc_owner_contrib', name: "Owner's Contributions", type: 'equity', parent: null, sort: 50 },
    { id: 'bkc_owner_draws', name: "Owner's Draws", type: 'equity', parent: null, sort: 51 },
  ];
  ...
  // A ledger deposit can be applied to an invoice (settles it; excluded from P&L income to avoid double counting)
  await sql`ALTER TABLE bk_transactions ADD COLUMN IF NOT EXISTS invoice_id TEXT REFERENCES invoices(id) ON DELETE SET NULL`;

  // Legacy owner-investment entries become equity transactions under Owner's Contributions
  await sql`UPDATE bk_transactions SET type = 'equity', category_id = COALESCE(category_id, 'bkc_owner_contrib') WHERE type = 'investment'`;

  // Stripe billing tables (subscriptions, payments, webhook events)
  await initBillingDb();
  // Sign-in codes, reset tokens, trusted devices
  await initAuthDb();
  // Product lines + classification rules
...
export async function getAppData() {
  ...
  const [settingsRows, clients, categories, services, invoiceRows, itemRows, installmentRows, projects, onlinePayments, attachments] = await Promise.all([
  ...
  return {
    clients: clients.map(c => ({ id: c.id, name: c.name, email: c.email, phone: c.phone, address: c.address, stripeCustomerId: c.stripe_customer_id || null })),
    ...
    invoices: invoiceRows.map(inv => ({
      ...
      recurringNext: toISODate(inv.recurring_next) || '',
      productLineId: inv.product_line_id || '',
      items: itemsByInvoice[inv.id] || [],
      installments: installmentsByInvoice[inv.id] || [],
      onlinePayments: onlinePayments[inv.id] || [],
      attachments: attachments[inv.id] || [],
    })),
    ...
    productLines: await listProductLines(),
    settings: {
      companyName: s.company_name || '',
      companyAddress: s.company_address || '',
      companyPhone: s.company_phone || '',
      taxState: s.tax_state || 'MD',
      taxLocalRate: s.tax_local_rate == null ? 3.2 : parseFloat(s.tax_local_rate),
      taxFiling: s.tax_filing || 'single',
      autoReminders: s.auto_reminders !== false,
    },
  };
}
...
export async function setAppData(data) {
  ...
  await sql`
    INSERT INTO settings (id, company_name, company_address, company_phone, tax_state, tax_local_rate, tax_filing, auto_reminders)
    VALUES (1, ${s.companyName || ''}, ${s.companyAddress || ''}, ${s.companyPhone || ''}, ${s.taxState || 'MD'}, ${s.taxLocalRate === '' || s.taxLocalRate == null ? 3.2 : Number(s.taxLocalRate)}, ${s.taxFiling || 'single'}, ${s.autoReminders !== false})
    ON CONFLICT (id) DO UPDATE SET
      company_name = EXCLUDED.company_name,
      company_address = EXCLUDED.company_address,
      company_phone = EXCLUDED.company_phone,
      tax_state = EXCLUDED.tax_state,
      tax_local_rate = EXCLUDED.tax_local_rate,
      tax_filing = EXCLUDED.tax_filing,
  ...
  for (const inv of invs) {
    ...
    await sql`
      INSERT INTO invoices (id, number, client_id, client_name, client_email, client_phone, client_address, project_id, due_date, notes, status, amount_paid, total, deposit, sent_at, paid_at, recurring, recurring_next, product_line_id)
      VALUES (
        ...
        ${toISODate(inv.sentAt) || null}, ${toISODate(inv.paidAt) || null}, ${inv.recurring || null}, ${toISODate(inv.recurringNext) || null}, ${inv.productLineId || null}
      )
      ON CONFLICT (id) DO UPDATE SET
        ...
        recurring_next = EXCLUDED.recurring_next,
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      await sql`
        ...
        VALUES (${inv.id}, ${item.description || ''}, ${item.qty || 1}, ${item.rate || 0}, ${item.amount || (parseFloat(item.qty) || 0) * (parseFloat(item.rate) || 0)}, ${i})
      `;
    }
  ...
  // Online (Stripe) payments are the source of truth for what they cover — never let a stale save undo them
