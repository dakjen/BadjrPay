...
export async function setAppData(data) {
  ...
  for (const inv of invs) {
    ...
    await sql`
      INSERT INTO invoices (id, number, client_id, client_name, client_email, client_phone, client_address, project_id, due_date, notes, status, amount_paid, total, deposit, sent_at, paid_at, recurring, recurring_next)
      VALUES (
        ...
        ${toISODate(inv.sentAt) || null}, ${toISODate(inv.paidAt) || null}, ${inv.recurring || null}, ${toISODate(inv.recurringNext) || null}
      )
      ON CONFLICT (id) DO UPDATE SET
