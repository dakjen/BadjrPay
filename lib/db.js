...
export async function getAppData() {
  ...
  for (const inst of installmentRows) {
    ...
    installmentsByInvoice[inst.invoice_id].push({
      ...
      dueDate: String(inst.due_date).slice(0, 10),
      status: inst.status,
      paidAt: inst.paid_at ? String(inst.paid_at).slice(0, 10) : null,
  ...
  return {
    ...
    projects: projects.map(p => ({ id: p.id, name: p.name, description: p.description, clientName: p.client_name, status: p.status, createdAt: p.created_at ? String(p.created_at).slice(0, 10) : null })),
    settings: {
...
export async function setAppData(data) {
  ...
  for (const inv of invs) {
    ...
    await sql`
      INSERT INTO invoices (id, number, client_id, client_name, client_email, client_phone, client_address, project_id, due_date, notes, status, amount_paid, total, deposit)
      VALUES (
      ...
      ON CONFLICT (id) DO UPDATE SET
        ...
        due_date = ${toISODate(inv.dueDate)},
